...
- **`api/telegram.js`** — Telegram webhook. Locked to `TELEGRAM_CHAT_ID`, verified via the `x-telegram-bot-api-secret-token` header (`TELEGRAM_WEBHOOK_SECRET`). Per message it fetches live WHOOP/Strava/weather server-side, builds the prompt via **`api/_prompt.js`** (a server-side mirror of `buildSystemPrompt()` + the schedule-block parser/appliers), calls Claude, applies any `schedule_set`/`schedule_update`, and appends the exchange to `coach_state`. Commands: `/start`, `/plan`, `/reset`. **`_prompt.js` must be kept in sync with the app's `buildSystemPrompt()`/`extractScheduleUpdates()`/`applyScheduleSet()`/`autoLogRidesFromStrava()` when either changes.** Setup + webhook registration: see DEPLOY.md → "Telegram bot".
